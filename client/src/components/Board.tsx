...
function TileCell({ id, name, color }: { id: number; name: string; color?: string }) {
  const { players } = useGame()
  const occupants = players.filter(p => p.position === id)
  return (
    <div className="relative flex items-end justify-center border border-white/10 bg-white/60 dark:bg-white/5 p-1 overflow-hidden" style={{
      backgroundImage: color ? `linear-gradient(180deg, ${color} 0 6px, transparent 6px)` : undefined,
      backgroundRepeat: 'no-repeat'
    }}>
      <div className="absolute inset-0 pointer-events-none" />
      <div className="text-[10px] text-center leading-tight px-1 opacity-80 select-none">{name}</div>
      <div className="absolute top-0 right-0 left-0 mx-auto flex gap-0.5 justify-center p-0.5">
        {occupants.map(o => (
          <span key={o.id} className="pill" title={o.name}>{tokenEmoji(o.token)}</span>
        ))}
      </div>
    </div>
  )
...
export default function Board() {
  ...
  return (
    <div className="mx-auto grid gap-2 sm:gap-3 md:gap-4" style={{
